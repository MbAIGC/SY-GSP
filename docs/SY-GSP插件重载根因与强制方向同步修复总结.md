...
本文档「3. 选边后循环暂停」一节中「LOCAL_OVER_REMOTE/REMOTE_OVER_LOCAL
两个强制方向在规划器和引擎里从未被实现」的表述不准确：规划器 `_applyOverride`
当时已实现逐路径强制语义，真正根因是引擎基准解析在其之前拦截；「策略 2/3
未消费强制模式」同样不准确——有基准时策略 2/3 正常走进规划器覆盖语义。
详见《SY-GSP同步收敛循环重构与CAS确认修正总结.md》。
